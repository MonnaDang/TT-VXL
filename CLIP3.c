...
void my_delay(){
   delay_ms(1);
   if(key4x4_read() == OK){
      if(key4x4.key == 0x0F) doi_mode = !doi_mode;
      else if((key4x4.key >= 1) && (key4x4.key <= 10)){
         if(!doi_mode) mode = key4x4.key;
         else speed = (int16)key4x4.key*50;
      }
   }
}
